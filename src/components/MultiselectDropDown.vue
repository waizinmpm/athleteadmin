    ...
    export default {
        ...
        props:{
            options: {
                type: Array,
                required: true
                // options array must contain id & name in object
            },
            limit: {
                type: Number,
                required: false,
        ...
        methods: {
            ...
            showSelectedItems() {
                this.selected_items_list = this.options.filter((opt) => this.value.find((v) => v == opt.id)).map((ele) => ele.name).join();
            },
            limit_reached(item){
                // parameter must be input value
                return this.selected.length == this.limit && this.selected.indexOf(item) === -1;
            },
            hideDropDown() {
