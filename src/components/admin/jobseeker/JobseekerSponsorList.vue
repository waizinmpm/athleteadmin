    ...
    <div @click="handleStatusToggle">   
        <div class="row">
            <div class="col-sm-12 p-0 searchform-one">
                <!--advanced search-->
				<h5 class="m-b-10 main-header">{{ $t('sponsor_list.jobseeker_title') }} </h5>
				<div class="inner-wrapper">
					...
					<div class="row">
						<div class="col-md-3">
							<label for="アスリート会員番号">{{ $t('sponsor_list.jobseeker_number') }}</label>
						...
						<div class="col-md-3">
							<label for="アスリート氏名">{{ $t('sponsor_list.jobseeker_name') }}</label>
							<input type="text" :placeholder="$t('sponsor_list.jobseeker_name')" class="form-control" v-model.trim="filteredData.jobseeker_name">
						</div>                        
					</div>
                    ...
                    <span class="tooltip-box" v-if="visible">
                        <table class="tooltiptext">
                            ...
                            <tbody>
								...
								<tr><td class="w-105">辞退</td><td>企業会員からの不採用又はアスリート会員の辞退。</td></tr>
								<tr><td class="w-105">スポンサー合意未請求</td><td>企業会員からの内定の連絡があったが、手数料を未請求。</td></tr>
...
export default {
	...
	data(){
		let sortOrders = {};
        let columns = this.$i18n.messages.en.sponsor_list.columns;
		...
		return {
			...
			base_url: "/v1/admin/sponsor-list/jobseeker",
			columns: columns,
